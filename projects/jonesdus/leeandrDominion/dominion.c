...
/**
 * List of card actions
 * 
 * TODO: figure out of the whole switch statement is removed
 * or just individual cards
 * */
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case feast:
    return feastCard(i, state, currentPlayer, temphand, x, choice1);
  case gardens:
    return -1;

  case mine:
    return mineCard(i, j, state, currentPlayer, choice1, choice2, handPos);
  case remodel:
    j = state->hand[currentPlayer][choice1]; //store card we will trash
